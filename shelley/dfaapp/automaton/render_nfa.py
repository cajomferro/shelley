...
from shelley.dfaapp.automaton import render_dfa
...
def epsilon_edges(nfa, states):
    ...
    while len(next_states) != len(states):
        ...
        next_states = states.union(
            states.union(nfa.multi_transition(states, None)))
        ...
        for ((src_st, inp), outs) in sorted(lvl.items()):
            for dst_st in outs:
                edges.append(
                    Step(
                        source=(src_st,
                                (nodes if inp is not None else next_nodes).get(
                                    src_st, 0)),
                        input=inp,
