...
def is_primitive(r):
  if isinstance(r, Concat) and is_primitive(r.left) and is_primitive(r.right):
    return True
...
def wrap(nfa):
  ...
  def tsx(st, a):
    if st is start:
      return frozenset({(1, nfa.start_state)}) if a is None else frozenset()
    if st is end:
      return frozenset()
    idx, st = st
    assert idx == 1
...
class GNFA:
  ...
  def as_graph(self):
    return self.states, dict((x, (y,)) for x,y in self.transitions.items())
  ...
  def step(gnfa, node=None):
    ...
    if node is None:
      ...
      if node == gnfa.start_state:
        raise ValueError(f"Node to remove {node} cannot be the start state.")
      if node == gnfa.end_state:
        raise ValueError(f"Node to remove {node} cannot be the end state.")
    ...
    for qi in si:
      for qj in sj:
        ...
        if result is not Nil:
          tsx[(qi,qj)] = result
    gnfa = GNFA(states, gnfa.alphabet, tsx, gnfa.start_state, gnfa.end_state)
    return node, gnfa
...
def regex_to_nfa(r, alphabet):
  ...
  if r is Nil:
    ...
    return regex_to_nfa(r.left, alphabet).concat(regex_to_nfa(r.right, alphabet))
  elif isinstance(r, Union):
    return regex_to_nfa(r.left, alphabet).union(regex_to_nfa(r.right, alphabet))
...
def save_nfa_to_regex_dot(n, prefix, dry_run=False, **kwargs):
  ...
  result.append(render_nfa.save_tex(w,
    prefix + "-wrapped-hl",
    dry_run=dry_run,
  ...
  steps = list(g.reduce())
  st = steps[0][0]
  result.append(save_tex(g, prefix,
    dry_run=dry_run,
    transition_name=tex_transition_name,
    **kwargs
  ))
  result.append(save_tex(g, prefix + "-hl",
    dry_run=dry_run, **get_highlights(g, [st])))
