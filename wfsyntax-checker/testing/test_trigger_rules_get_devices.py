...
from _local.devices import get_devices_from_trigger_rule
...
class TestGetDevicesAsSet(unittest.TestCase):
    ...
    def test_sequence(self):
        self.assertEqual({'b', 't'},  # order is not important
                         get_devices_from_trigger_rule(TriggerRuleSequence(
                             TriggerRuleEvent("b", "released"),
                             TriggerRuleEvent("t", "canceled"))))

    def test_sequence_3_elems(self):
        self.assertEqual({'b', 't', 'ledA'},  # order is not important
                         get_devices_from_trigger_rule(
                             TriggerRuleSequence(
                                 TriggerRuleEvent("b", "released"),
                                 TriggerRuleSequence(
                                     TriggerRuleEvent("t", "canceled"),
    ...
    def test_sequence_4_elems(self):
        self.assertEqual({'ledB', 'b', 't', 'ledA'},  # order is not important
                         get_devices_from_trigger_rule(
                             TriggerRuleSequence(
                                 TriggerRuleSequence(
    ...
    def test_sequence_choice(self):
        self.assertEqual({'b', 'ledA', 't'},  # order is not important
                         get_devices_from_trigger_rule(
                             TriggerRuleSequence(
                                 TriggerRuleChoice(
                                     TriggerRuleEvent("b", "released"),
    ...
    def test_sequence_choice_choice(self):
        self.assertEqual({'b', 'ledB', 'ledA', 't'},  # order is not important
                         get_devices_from_trigger_rule(
                             TriggerRuleSequence(
                                 TriggerRuleChoice(
                                     TriggerRuleEvent("b", "released"),
    ...
    def test_desk_lamp(self):
        self.assertEqual(
            {'b', 't', 'ledB', 'ledA'},
            get_devices_from_trigger_rule(self.triggers_result[0].trigger_rule))

        self.assertEqual({'t', 'ledB'},
                         get_devices_from_trigger_rule(self.triggers_result[1].trigger_rule))
