...
class SmartButtonDriver(Device):  # implements device SmartButton
    ...
    def __init__(self, driver_id: str, timer: Timer, button: Button):
        ...
        self.button.subscribe(
            device_callback=self.on_button_pressed, event=Button.PRESSED
        )
    ...
    def on_button_pressed(self):
        ...
        self.timer.start(
            5
        )  # this is the predefined duration for the buttonPressedLong (5 seconds)

    @callback(Timer.STARTED)
    def on_timer_started(self):
        self.timer.subscribe(
            device_callback=self.on_timer_timeout_long, event=Timer.TIMEOUT
        )
        self.button.subscribe(
            device_callback=self.on_button_released, event=Button.RELEASED
        )
    ...
    def on_button_released(self):
        self.timer.subscribe(
            device_callback=self.on_timer_canceled, event=Timer.CANCELED
        )
        self.timer.cancel()
    ...
    def on_timer_canceled(self):
        self.timer.subscribe(
            device_callback=self.on_timer_started_twice, event=Timer.STARTED
        )
        self.timer.start(
            0.5
        )  # this is the predefined duration for the buttonPressedOnce (half a second)

    @callback(Timer.STARTED)
    def on_timer_started_twice(self):
        self.timer.subscribe(
            device_callback=self.on_timer_timeout_twice, event=Timer.TIMEOUT
        )
        self.button.subscribe(
            device_callback=self.on_button_pressed_twice, event=Button.PRESSED
        )
    ...
    def on_button_pressed_twice(self):
        self.timer.cancel()
        self.button.subscribe(
            device_callback=self.on_button_released_twice, event=Button.RELEASED
        )
