...
SYSTEM_START_EVENT_REQUIRED = "At least one start event must be specified." # this never occurs if we have a good parser?
SYSTEM_FINAL_EVENT_REQUIRED = "At least one final event must be specified." # this never occurs if we have a good parser?
SYSTEM_START_STATE_KEYWORD_RESERVED = lambda x: f"Start state '{x}' cannot have the same name as an event."
...
# SUO2: operations that end in a sink state (point of no return). Is this equivalent to the concept of deadlock?
UNUSABLE_OPERATIONS_NO_YIELD_POINT = lambda x: f"These operations do not reach a yield point: {', '.join(x)}"

# Invalid integration
TRIGGER_NONE_SMALLEST_ERROR = "dec_seq can only be none if failure is empty, which cannot be"
TRIGGER_NONE_INDEX = "The index can only be None if the behavior is empty, which should never happen"
TRIGGER_UNEXPECTED_VALID_INTEGRATION = lambda x: f"Expecting an invalid projection, but got: {x}"
TRIGGER_REPORT = lambda macro, micro, micro_hl, \
...
UNDECLARED_OPERATION_IN_SUBSYSTEM = lambda x, y: f"System operation '{x}' contains undeclared subsystem operations: {y}"
