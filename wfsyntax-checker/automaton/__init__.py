...
class DFA:
  ...
  def union(dfa1, dfa2):
    ...
    return DFA(
      states = frozenset(product(dfa1.states, dfa2.states)),
  ...
  def make_char(cls, alphabet, char):
    ...
    return cls(
      states=[0, 1, 2],
      alphabet=alphabet,
      transition_func=lambda q, a: 1 if q == 0 and a == char else 2,
      start_state=0,
...
class NFA:
  ...
  def epsilon(self, states):
    for idx, st in enumerate(states, 1):
      if st not in self.states:
        raise ValueError(f"Error at state #{idx}={repr(st)} not in states={self.states}")
    states = set(states)
    while True:
      count = len(states)
  ...
  def union(nfa1, nfa2):
    ...
    def transition(st, a):
      idx, st = st
      if idx == 2:
        return frozenset({(0, nfa1.start_state), (1, nfa2.start_state)}) if a is None else frozenset()
  ...
  def shuffle(nfa1, nfa2):
    def transition(st, a):
      ...
      result = set()
      if a is None:
        result.update((st1, st2) for st1 in nfa1.transition_func(st1, a))
        result.update((st1, st2) for st2 in nfa2.transition_func(st2, a))
        return result
  ...
  def concat(nfa1, nfa2):
    ...
    def transition(st, a):
      ...
      if idx == 0 and nfa1.accepted_states(st) and a is None:
        return frozenset({(1, nfa2.start_state)}).union(frozenset(map(lambda x:(0,x), nfa1.transition_func(st, a))))
      return frozenset(tag(idx, tsx[idx](st, a)))
  ...
  def from_transition_edges(cls, edges, **kwargs):
    ...
    if "alphabet" not in kwargs:
      kwargs["alphabet"] = set(k for (_,elems,_) in edges for k in elems)
    return cls(
      transition_func = cls.transition_edges(edges),
      **kwargs)
