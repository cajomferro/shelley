...
from _local.devices import get_devices_as_list
...
class TestGetDevicesAsSet(unittest.TestCase):
    ...
    def test_sequence(self):
        self.assertEqual([['b', 't']],
                         get_devices_as_list(TriggerRuleSequence(
    ...
    def test_desk_lamp(self):
        self.assertEqual(
            [['b', 'b', 't', 'ledB', 'ledA'], ['b', 'b', 't', 'ledA', 'ledB'], ['t', 'ledB', 'ledA'],
             ['t', 'ledA', 'ledB']],
            get_devices_as_list(self.triggers_result[0].trigger_rule, self.declared_components))

        self.assertEqual([['t', 'ledB']],
                         get_devices_as_list(self.triggers_result[1].trigger_rule, self.declared_components))
