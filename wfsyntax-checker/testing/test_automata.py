...
def replace(r, rules):
  ...
  def do_subst(r, attr):
    ...
    if isinstance(child, Char):
      ...
      return
    elif isinstance(child, Concat) or isinstance(child, Union) or isinstance(child, Star):
...
def main(fs):
  ...
  fs.save_nfa_dot(B.shuffle(LA).shuffle(LB).shuffle(T), "dev", state_name=render_state_name)
  ...
  T_STANDBY2 = L("s2")
  lbl_to_rex = {
    T_LEVEL1: Concat.from_list(map(Char, [B_P, B_R, LA_ON, T_S])),
    T_LEVEL2: Concat.from_list([
      Char(B_P),
      Char(B_R),
      And(Char(T_C), Char(LB_ON)),
      Char(T_S),
    ]),
    T_STANDBY1: concat(Char(T_T), Char(LA_OFF)),
    T_STANDBY2: concat(
      Union(
        Concat.from_list(map(Char, [B_P, B_R, T_C])),
        Char(T_T)
      ),
  ...
  ALL = (tuple(B.alphabet) + tuple(LA.alphabet) +
    tuple(LB.alphabet) + tuple(T.alphabet))
