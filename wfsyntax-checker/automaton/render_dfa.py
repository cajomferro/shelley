...
def build(m, get_graph=None, highlight_nodes=(), highlight_edges=(), extra_edge_style=lambda src, dst, kwargs:(), state_name=str, transition_name=str, initial_orientation=None, edge_stack=False, fig_only=False):
  dot = Digraph()
  dot.attr(
  ...
  for ((src, dst), chars) in sorted(edges.items()):
    ...
    dot.edge(state_name(src), state_name(dst), label=char_sep.join(chars), **kwargs)
...
def save_reduction_dot(a, prefix, inputs, **kwargs):
  ...
  if dry_run:
    ...
    def do_save(counter):
      result.append(d.save("%s%d.dot" % (prefix, counter), directory=os.getcwd()))
      return counter + 1
  ...
  for i, s in zip(inputs, step(a, inputs)):
    d = build(a, highlight_edges=[(prev, s)], highlight_nodes=[s], **kwargs)
    counter = do_save(counter)
    prev = s
  return result
